...
function isAllTrue(array, fn) {
  ...
  return true;
  // Если так нельзя, то можно с for
}
...
function calculator(number = 0) {
  ...
  return {
    sum: (...args) => args.reduce((p, c) => p + c, number),
    dif: (...args) => args.reduce((p, c) => p - c, number),
    div: (...args) =>
      args.reduce((p, c) => {
        if (c === 0) throw new Error('division by 0');
        return p / c;
      }, number),
    mul: (...args) => args.reduce((p, c) => p * c, number),
  };
}
