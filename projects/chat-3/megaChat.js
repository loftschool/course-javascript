...
export default class MegaChat {
  ...
  async onLogin(name) {
    ...
    this.ui.userPhoto.set(`/chat-3/photos/${name}.png?t=${Date.now()}`);
  }
  ...
  onMessage({ type, from, data }) {
    ...
    if (type === 'hello') {
      ...
      for (const avatar of avatars) {
        avatar.style.backgroundImage = `url(/chat-3/photos/${
          data.name
        }.png?t=${Date.now()})`;
      }
    }
